...
sub ConfigureRAF(adIface)
    ...
    if videoNode.content.ads.adFormat = "VAST"
        adIface.setAdUrl(videoNode.content.ads.uri)
        adPods = []
        if videoNode.content.ads.cue_points <> invalid
            for each adType in videoNode.content.ads.cue_points
                if adType.enable = true
                    if adType.type = "PREROLL"
                        adPod = adIface.GetAds()[0]
                        adPod.renderSequence = "preroll"
                        adPod.renderTime = 0
                        adPod.ads[0].id = 0
                        adPods.Push(adPod)
                    else if adType.type = "MIDROLL"
                        if videoNode.content.is_live = false
                            for each position in adType.position
                                adPod = adIface.GetAds()[0]
                                adPod.renderSequence = "midroll"
                                adPod.renderTime = position.toint()
                                adPod.ads[0].id = position
                                adPods.Push(adPod)
                            end for
                        else
                            ? adType
                            m.midrolls = adType
                            m.midrolls.nextAdTimer = 0
                        end if
                    else if adType.type = "POSTROLL"
                        adPod = adIface.GetAds()[0]
                        adPod.renderSequence = "postroll"
                        adPod.ads[0].id = adPod.duration
                        adPods.Push(adPod)
                    end if
                end if
            end for
        ...
        if videoNode.content.ads.uri <> invalid
            adIface.setAdUrl(videoNode.content.ads.uri)
        ...
        ' if it is some video playback event
            ...
            if adIface.sgdex_flag_StitchedAdsInit_was_called = Invalid
                ...
                if msg.GetField() = "position"
                    ...
                    if adPod = invalid and m.midrolls <> invalid and videoView.content.is_live <> invalid and videoView.content.is_live = true and videoView.content.manage_ads = true
                        if m.midrolls <> invalid and m.midrolls.enable = true and (curPos = m.midrolls.ad_frequency_sec.toint() or curPos > m.midrolls.ad_frequency_sec.toint()) and (CurPos >  m.midrolls.nextAdTimer)
                            adPod = adIface.GetAds()
                            if adPod <> invalid and adPod.Count() > 0
                                m.midrolls.nextAdTimer = m.midrolls.ad_frequency_sec.toint()+1
                            else
                                m.midrolls.nextAdTimer = curPos + m.midrolls.ad_frequency_sec.toint()
                            end if
                        end if
                    end if
...
' if adPod <> invalid
'     if adPod.rendersequence = "midroll" and videoView.content.is_live = true
'         ads = []
'         data = adPod
'         data.rendertime = data.rendertime + data.rendertime
'         ads.push(data)
'         adIface.importAds(ads)
'     end if
' end if