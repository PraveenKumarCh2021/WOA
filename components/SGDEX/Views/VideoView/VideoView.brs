...
sub OnVideoStateChanged(event as Object)
    ...
    if m.video <> Invalid then
        if state <> "buffering" and state <> "error"
            data = {
                video : m.video
                content : m.video.content
            }
            m.global.setFields({trp : data})
        end if
...
Sub OnDurationChanged(event as Object)
    ...
    if m.video.content.trp_intervel_sec <> invalid
...
sub OnPositionChanged(event as Object)
    ...
    if  m.video <> invalid and m.video.content.manage_trp <> invalid and m.video.content.manage_trp = true and (position >  m.lastSent+m.video.content.trp_intervel_sec)
        data = {
            video : m.video
            content : m.video.content
        }
